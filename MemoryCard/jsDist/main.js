...
let prepare = {
    cards: Array.from(document.querySelectorAll('.memoryCard')).map((card) => {
        ...
        return ({
            element: card,
            imgFrontFaceSrc: ((_a = card.querySelector('.front-face img')) === null || _a === void 0 ? void 0 : _a.src) || '',
        });
    }),
...
function flipCard() {
    ...
    if (!card || !gameStarted || this.classList.contains('flipped') || matchedCards.find((c) => c === card))
        return;
...
function checkForMatch() {
    ...
    let Match = firstCard.imgFrontFaceSrc === secondCard.imgFrontFaceSrc;
    if (Match) {
...
function updateProgressBar() {
    const progressPercentage = ((matchedCards.length / 2) / prepare.totalPairs) * 100;
...
function shuffleCards() {
    const images = prepare.cards.map((card) => card.imgFrontFaceSrc);
    ...
    prepare.cards.forEach((card, index) => {
        const img = card.element.querySelector('.front-face img');
